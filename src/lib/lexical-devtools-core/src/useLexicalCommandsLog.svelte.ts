...
export function useLexicalCommandsLog(editor: LexicalEditor): () => LexicalCommandLog {
	let loggedCommands = $state<LexicalCommandLog>([]);

	$effect(() => {
		return registerLexicalCommandLogger(editor, (v) => {
			v(loggedCommands);
		});
	});
