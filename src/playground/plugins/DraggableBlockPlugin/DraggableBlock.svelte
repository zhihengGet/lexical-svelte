<script context="module" lang="ts">
	...
	const DRAGGABLE_BLOCK_MENU_class = 'draggable-block-menu';
	...
	function isOnMenu(element: HTMLElement): boolean {
		return !!element.closest(`.${DRAGGABLE_BLOCK_MENU_class}`);
	}
...
<script lang="ts">
	let { editor, anchorElem, isEditable } = $props<{
		editor: LexicalEditor;
		anchorElem: HTMLElement;
		isEditable: boolean;
	}>();
	let isMouseover = $state({ state: false });
	...
	useEffect(() => {
		...
		scrollerElem?.parentElement?.addEventListener('mouseleave', onMouseLeave);
...
<div
	class="icon draggable-block-menu bg-purple z-2"
	bind:this={menuRef.current}
	onmouseover={() => {
		isMouseover.state = true;
	}}
	onmouseleave={() => {
		isMouseover.state = false;
	}}
	draggable={true}
